...
density_color = bpy.data.materials.new(name="Exoplanet Ice Surface")
density_color.use_nodes = True
nodes = density_color.node_tree.nodes
links = density_color.node_tree.links
bsdf =  density_color.node_tree.nodes['Principled BSDF']

color_ramp = nodes.new("ShaderNodeValToRGB")
color_ramp.location = (-500,300)
 
xyz = nodes.new("ShaderNodeSeparateXYZ")
xyz.location = (-750, 300)

links.new(xyz.outputs["Z"], color_ramp.inputs["Fac"])

mapping= nodes.new("ShaderNodeMapping")
mapping.location = (-900,300)
links.new(mapping.outputs["Vector"], xyz.inputs["Vector"])
...
circle.data.materials.append(density_color)
